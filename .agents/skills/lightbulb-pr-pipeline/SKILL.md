...
- Parent/root owns queue order, GitHub mutation, final merge, issue closeout, and next-slice dispatch.
...
4. Maker implements, verifies, commits, pushes, and opens a draft or ready PR against `dev` when authorized.
...
7. Request external review using the available GitHub/Codex path, then poll PR reviews, comments, checks, and merge state until a decision, timeout, or infrastructure blocker is clear.
8. Feed actionable comments and failing checks back to the maker thread. Maker fixes in the same branch, reruns focused verification, pushes, and returns evidence.
9. Repeat review polling plus maker fixes until approvals are present, no blocking reviewer findings remain, checks are green or explicitly infra-blocked, and visual evidence is attached.
10. Parent re-reads the PR, confirms the final head SHA, verifies the merge gate, merges when authorized, posts issue proof comments when mutating GitHub, then starts the next queued slice in a new maker thread.

## Hard Stops

- Conflicting user or worker changes touch the same files without a clear owner.
- The PR has unresolved requested changes, unknown failing checks, merge conflicts, missing visual evidence, or no approval path.
...
- checks, approvals, visual evidence location, and unresolved caveats
